...
# from generators.data import SeqDataset, SeqDataset, Seq2SeqDataset, BertRecTrainDataset
from utils.utils import unzip_data, concat_data, unzip_data_with_attention_marker
from generators.data_with_att import SeqDataset_att, Seq2SeqDataset_att, BertRecTrainDataset_att
...
class Generator(object):
    ...
    def _load_dataset(self):
        ...
        f = open('./data/%s/processed/%s.txt' % (self.dataset, self.inter_file), 'r')
    ...
    def make_trainloader(self):
        ...
        train_dataset, attention_marker = unzip_data_with_attention_marker(self.dataset, self.train, aug=self.args.aug, aug_num=self.args.aug_seq_len)
        self.train_dataset = SeqDataset_att(train_dataset, attention_marker, self.item_num, self.args.max_len, self.args.train_neg)
    ...
    def make_evalloader(self, test=False):
        ...
        self.eval_dataset = SeqDataset_att(eval_dataset, self.item_num, self.args.max_len, self.args.test_neg)
...
class Seq2SeqGenerator(Generator):
    ...
    def make_trainloader(self):
        train_dataset, attention_marker = unzip_data_with_attention_marker(self.dataset, self.train, aug=self.args.aug, aug_num=self.args.aug_seq_len)
        self.train_dataset = Seq2SeqDataset_att(self.args, train_dataset, attention_marker, self.item_num, self.args.max_len, self.args.train_neg)
