...
from generators.distill_generator import Generator, SeqGenerator, Seq2SeqGenerator
from trainers.distill_seq_trainer import SeqTrainer
...
parser.add_argument("--model_name", 
                    default="dt_llm_sasrec",
                    choices=[
                    "dt_llm_bert4rec", "dt_llm_gru4rec","dt_llm_sasrec",
                    ],
                    type=str, 
                    required=False,
                    help="model name")
...
# new
parser.add_argument("--marker_dim",
                    default=16,
                    type=int,
                    help="the dimension of marker")
parser.add_argument("--topk",
                    default=10,
                    type=int,
                    help="the number of top-k items")
parser.add_argument("--marker_max_num",
                    default=10,
                    type=int,
                    help="the maximum number of markers")
parser.add_argument("--weight_ranking_loss",
                    default=0,
                    type=float,
                    help="the weight of ranking distillation loss")
parser.add_argument("--weight_dros_loss",
                    default=0,
                    type=float,
                    help="the weight of dros loss")
...
def main():
    ...
    if args.model_name in ['dt_llm_gru4rec']:
        generator = Generator(args, logger, device)
    elif args.model_name in ["dt_llm_bert4rec"]:
        generator = BertGenerator(args, logger, device)
    elif args.model_name in ["dt_llm_sasrec"]:
