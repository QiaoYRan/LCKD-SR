...
class SASRecBackbone(nn.Module):
    ...
    def forward(self, seqs, log_seqs): # log_seqs is used for attention mask
...
class SASRec_seq(SASRec):
    ...
    def log2feats_with_attention_marker(self, log_seqs, positions, marked_seq):
        # Get the representation of given sequence after self-attention with attention marker
        seqs = self._get_embedding(log_seqs)
        seqs *= self.item_emb.embedding_dim ** 0.5
        seqs += self.pos_emb(positions.long())
        seqs = self.emb_dropout(seqs)
        ...
        attended_seqs = self.self_attention_module(seqs, log_seqs, marked_seq)
        # attention_marker is a tensor of shape (batch_size, marker_max_num)
        # each element in attention_marker is an item_id


        log_feats = self.backbone(attended_seqs, log_seqs)
    ...
    def forward(self, 
                ...
                **kwargs):
        # apply the seq-to-seq loss
        log_feats = self.log2feats_with_attention_marker(seq, positions, marked_seq)
        pos_embs = self._get_embedding(pos) # (bs, max_seq_len, hidden_size)
        neg_embs = self._get_embedding(neg) # (bs, max_seq_len, hidden_size)

        pos_logits = (log_feats * pos_embs).sum(dim=-1) # size: (bs, max_seq_len)
        neg_logits = (log_feats * neg_embs).sum(dim=-1)
