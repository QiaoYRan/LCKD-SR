...
from utils.utils import metric_len_5group, metric_pop_5group, calculate_ranking_distillation_loss
...
class SeqTrainer(Trainer):
    ...
    def _train_one_epoch(self, epoch):
        ...
        for batch in prog_iter:
            ...
            loss, all_items_logits = self.model(**inputs)
            # implement the ranking distillation loss in utils.py
            # loss += calculate_ranking_distillation_loss(log_feats, all_items_logits, inputs["pos"])
            ranking_loss = calculate_ranking_distillation_loss(self.args, inputs['pos'], inputs['neg'], all_items_logits)
            loss += ranking_loss
            loss.backward()
